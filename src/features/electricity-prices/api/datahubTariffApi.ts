...
function getHourPrice(record: TariffRecord, hour: number): number {
...
export async function fetchHourlyTariff(glnNumber: string): Promise<number[]> {
  ...
  url.searchParams.set('filter', JSON.stringify({ GLN_Number: [glnNumber], ChargeType: ['D03'] }))
  ...
  for (const record of latestByCode.values()) {
    for (let h = 0; h < 24; h++) {
      hourly[h] = Math.round((hourly[h] + getHourPrice(record, h)) * 10000) / 10000
    }
  }
